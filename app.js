...
socket.on("join-result", (data)=>{
    ...
    if( data.status ){
      ...
      $("#gacha").style.display = "block";         // チャットを表示
    }
    //------------------------
    // できなかった
    //------------------------
...
document.querySelector("#btn-drop").addEventListener("click",()=>{
    ...
    .then((json)=>{
        if(json["status"]){
            ...
            name.innerHTML=`<strong>${json['result']['rarity']}　${json['result']['name']}</strong>`;
            if(json['result']['rarity']=="R"||json['result']['rarity']=="SR"){
                // Socket.ioサーバへ送信
                socket.emit("post", {text: "が"+json['result']['rarity']+"の"+json['result']['name']+"を引きました", token:IAM.token});
            }
        }
        else{
...
function gotoSTEP1(){
    ...
    $("#inputmyname").style.display = "none";     // 名前入力を非表示
    $("#gacha").style.display = "none";            // ガチャを非表示
...
function addMessage(msg, is_me=false){
    ...
    // マスターの発言
    if( msg.token === 0 ){
      li.innerHTML = `${time} <span class="msg-master"><span class="name">${name}</span>&gt; ${msg.text}</span>`;
    }
    // 自分の発言
    else if( is_me ){
      li.innerHTML = `${time} <span class="msg-me"><span class="name">あなた</span>${msg.text}</span>`;
    }
    // それ以外の発言
    else{
      li.innerHTML = `${time} <span class="msg-member"><span class="name">${name}</span>さん${msg.text}</span>`;
    }
